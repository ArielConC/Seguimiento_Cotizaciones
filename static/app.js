...
function ageBadge(q){const days=Number(q.quote_age_days||0);return `<span class="age-badge${days>90?' stale':''}">${days>90?'90+':days} ${t('days')}</span>`;}
function activityCell(q){return `${fmtDT(q.last_activity_at)}${q.activity_is_initial?`<span class="initial-badge">${t('initial_import')}</span>`:''}`;}
function trackingLegend(){const labels=state.language==='es'?['Pending 0–8 días','Pending 9–14 días','Pending 15+ días','PO','Lost']:['Pending 0–8 days','Pending 9–14 days','Pending 15+ days','PO','Lost'];const colors=['white','orange','red','green','purple'];return `<div class="row-legend"><b>${t('color_legend')}:</b>${labels.map((label,i)=>`<span><i class="legend-${colors[i]}"></i>${label}</span>`).join('')}</div>`;}
function managementTable(rows){
  if(!rows.length)return `<div class="empty">${t('no_records')}</div>${trackingLegend()}`;
  return `<table><thead><tr><th>${t('priority')}</th><th>${t('date')}</th><th>${t('reference')}</th><th>${t('distributor_code')}</th><th>${t('end_user')}</th><th>${t('order')}</th><th>${t('status')}</th><th>${t('quote_age')}</th><th>${t('last_activity')}</th><th></th></tr></thead><tbody>${rows.map(q=>`<tr class="${trackingClass(q)}"><td><span class="priority ${q.priority.toLowerCase()}">${q.priority}</span></td><td>${fmtDate(q.quote_date)}</td><td class="folio">${esc(q.folio)}</td><td>${state.system==='special'?'—':esc(q.distributor_code)||t('no_code')}</td><td>${esc(state.system==='special'?q.receptor:q.end_user)||'—'}</td><td>${esc(q.customer_order)||'—'}</td><td>${statusBadges(q)}</td><td>${ageBadge(q)}</td><td>${activityCell(q)}</td><td>${rowActions(q)}</td></tr>`).join('')}</tbody></table>${trackingLegend()}`;
}
function quoteTable(rows,compact=false,managed=false){if(!rows.length)return `<div class="empty">${t('no_records')}</div>`;const showLoss=!compact&&state.view==='lost';const showPo=!compact&&state.view==='po';const rowClass=q=>managed?` class="${trackingClass(q)}"`:'';
  if(state.system==='special')return `<table><thead><tr><th>${t('priority')}</th><th>${t('date')}</th><th>${t('reference')}</th><th>${t('end_user')}</th><th>${t('rank')}</th><th>Code</th><th>${t('model')}</th><th>${t('quantity')}</th><th>${t('unit_price')} (JPY)</th>${showPo?`<th>${t('po_date')}</th><th>${t('po_total')} (JPY)</th>`:''}<th>${t('status')}</th>${showLoss?`<th>${t('reason')}</th>`:''}${compact?'':'<th></th>'}</tr></thead><tbody>${rows.map(q=>`<tr${rowClass(q)}><td><span class="priority ${q.priority.toLowerCase()}">${q.priority}</span></td><td>${fmtDate(q.quote_date)}</td><td class="folio">${esc(q.folio)}</td><td>${esc(q.receptor)}</td><td>${esc(q.rank)||'—'}</td><td>${esc(q.code)}</td><td>${esc(q.description)}</td><td>${Number(q.quantity).toLocaleString()}</td><td class="money">${money(q.unit_price)}</td>${showPo?`<td>${fmtDate(q.po_date)}</td><td class="money">${q.po_total_usd?money(q.po_total_usd):'—'}</td>`:''}<td>${statusBadges(q)}</td>${showLoss?`<td>${esc(q.loss_reason)||'—'}</td>`:''}${compact?'':`<td>${rowActions(q)}</td>`}</tr>`).join('')}</tbody></table>${managed?trackingLegend():''}`;
  return `<table><thead><tr><th>${t('priority')}</th><th>${t('date')}</th><th>${t('reference')}</th><th>${t('distributor_code')}</th><th>${t('end_user')}</th><th>${t('dist_agent')}</th><th>${t('nt_agent')}</th><th>${t('quoted_total')} (USD)</th><th>${t('order')}</th>${showPo?`<th>${t('po_date')}</th><th>${t('po_total')} (USD)</th>`:''}<th>${t('status')}</th>${showLoss?`<th>${t('reason')}</th>`:''}${compact?'':'<th></th>'}</tr></thead><tbody>${rows.map(q=>`<tr${rowClass(q)}><td><span class="priority ${q.priority.toLowerCase()}">${q.priority}</span></td><td>${fmtDate(q.quote_date)}</td><td class="folio">${esc(q.folio)}</td><td>${esc(q.distributor_code)||t('no_code')}</td><td>${esc(q.end_user)||'—'}</td><td>${esc(q.distributor_agent)||'—'}</td><td>${esc(q.nt_agent)||'—'}</td><td class="money">${money(q.total_usd)}</td><td>${esc(q.customer_order)||'—'}</td>${showPo?`<td>${fmtDate(q.po_date)}</td><td class="money">${q.po_total_usd?money(q.po_total_usd):'—'}</td>`:''}<td>${statusBadges(q)}</td>${showLoss?`<td>${esc(q.loss_reason)||'—'}</td>`:''}${compact?'':`<td>${rowActions(q)}</td>`}</tr>`).join('')}</tbody></table>${managed?trackingLegend():''}`;
}
